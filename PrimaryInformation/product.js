...
router.post('/', async (req, res) => {
  try {
    const { firstName, lastName, age, birthSex } = req.body;
    console.log('1');
    const productId = await index.createProduct(firstName, lastName, age, birthSex);
    console.log('2');
...
router.post('/data', async (req, res) => {
  try {
    ...
    async function processEachBatch(batch) {
        ...
        const { firstName, lastName, age, birthSex } = item;
        const productIds = await index.createProduct(firstName, lastName, age, birthSex);
        count++;
...
router.put('/:id', async (req, res) => {
  try {
    const productId = req.params.id;
    const { firstName, lastName, age, birthSex } = req.body;
    const updatedProduct = await index.updateProduct(productId, firstName, lastName, age, birthSex);
