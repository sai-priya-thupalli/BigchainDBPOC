...
const conn = new driver.Connection('http://localhost:9986/api/v1/');

// Create a product
async function createProduct(address, city, state, zipcode, policyRole) {
  try {
    const assets = {
      address: address,
      city: city,
...
async function updateProduct(id, address, city, state, zipcode, policyRole) {
  try {
    const asset = await conn.searchAssets(id);
